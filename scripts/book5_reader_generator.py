#!/usr/bin/env python3
"""
Blood Craft Book 5 Reader Generator

This script parses the Book5-Full.md file and generates an interactive
HTML book reader for Book 5 (Chapters 51-62).
"""
...
class BookStructure:
    ...
    def __init__(self, title: str = "Blood Craft: Book Five - Rising Stars"):
        self.title = title
...
def parse_book5(file_path: str) -> BookStructure:
    """Parse the Book5-Full.md file."""
...
<body>
    ...
    <script>
        ...
        function init() {
            ...
            const savedPage = localStorage.getItem('bloodcraft_book5_current_page');
            if (savedPage) {
        ...
        function displayPage(pageIndex) {
            ...
            localStorage.setItem('bloodcraft_book5_current_page', page.global_page_number);
...
def main():
    ...
    # Paths
    narrative_path = 'canonical-version/Book5-Full.md'
    output_dir = 'canonical-version'
    html_output = os.path.join(output_dir, 'Book5-Reader.html')
    metadata_output = os.path.join(output_dir, 'book5-structure.json')
    
    if not os.path.exists(narrative_path):
        print(f"❌ Error: {narrative_path} not found!")
        sys.exit(1)
    
    print("🔍 Parsing Book 5 narrative...")
    book = parse_book5(narrative_path)
    ...
    print(f"\n✨ Done! Open '{html_output}' in your browser to read Book 5.")
